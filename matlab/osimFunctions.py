...
def setupScaleTool(pathList, filename, subName, mass, height, age):
    ...
    MP.find('marker_file').text = datDir + '/' + filename + '.trc'
    ...
    MP.find('output_marker_file').text = resPath + '/' + name + 'markersScaled.osim'
...
def calculateCrankAngle(path, filename, rotateFlag):
    ...
    ### Call ROTATEMARKERDATA function
    trajLabels, trajData, markers = rotateMarkerData(path, filename, rotateFlag)
    ...
    lSpindle = np.vstack((lSpindle, zR))
    ...
    return angleTdcRight, angleGlobalRight, angleGlobalLeft, rSpindle, lSpindle, markers
...
def writeExternalLoadFile(path, filename, rotateFlag, crankLength):
